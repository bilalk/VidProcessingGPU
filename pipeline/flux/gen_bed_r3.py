"""gen_bed_r3.py — procedural music bed for R3 workspace.
Adapted from gen_bed.py — outputs to /root/reels_r3/bed.wav"""
import numpy as np
import wave

SR = 44100
DUR = 180.0   # 3-minute loop
N = int(SR * DUR)
mix = np.zeros(N)
...
import os
os.makedirs('/root/reels_r3', exist_ok=True)
with wave.open('/root/reels_r3/bed.wav', 'wb') as w:
    w.setnchannels(2)
    w.setsampwidth(2)
    w.setframerate(SR)
    w.writeframes(pcm.tobytes())
print('BED_OK /root/reels_r3/bed.wav', flush=True)
