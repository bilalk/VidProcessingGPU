#!/usr/bin/env python
# run_ltx_v2.py - full-fidelity LTX-2.3 bf16 + 20 steps + spatial upsampler.
# Renders mobile (768x1344) and desktop (1344x768) natively, assembles genuine 16:9 (no blur).
import json, os, sys, time, subprocess, threading, shutil, urllib.request, urllib.parse, concurrent.futures, re
...
DONE = os.path.join(ROOT, "done_v2")
...
for d in (CLIPS_M, CLIPS_D, OUT_M, OUT_D, BUILD, LOGS, DONE):
...
def make_ass(r, ev, play_w, play_h):
...
def horizontal_prompt(s):
    # swap vertical -> horizontal suffix for desktop rendering
...
def build_static_recap(img, out, frames, tw, th, zoom):
    vf = f"scale={tw*1.0}:{th*1.0}:force_original_aspect_ratio=increase,crop={tw}:{th},{zoom.format(n=frames)}"
    run_cmd(['ffmpeg','-y','-loop','1','-i',img,'-vf',vf,'-frames:v',str(frames),
             '-c:v','libx264','-preset','veryfast','-crf','19','-pix_fmt','yuv420p',out])

def assemble(r, clips_dir, out_dir, tw, th):
    rid=r['id']
    ...
    # zoom for recap (mobile vs desktop appropriate)
    ...
    for i,(pstart,pend,k) in enumerate(panels):
        ...
        if k<3:
            clip=os.path.join(clips_dir,f"{rid}_p{k+1}.mp4")
...
def ltx_workflow(first, last, prompt, width, height, frames, seed, prefix):
    ...
    wf = {
        ...
        "24":{"class_type":"LTXVScheduler","inputs":{"steps":20,"max_shift":2.05,"base_shift":0.95,"stretch":True,"terminal":0.1}},
        ...
        "40":{"class_type":"LatentUpscaleModelLoader","inputs":{"model_name":SPATIAL}},
        "41":{"class_type":"LTXVLatentUpsampler","inputs":{"samples":["27",0],"upscale_model":["40",0],"vae":["3",2]}},
        "28":{"class_type":"LTXVCropGuides","inputs":{"positive":["19",0],"negative":["19",1],"latent":["41",0]}},
...
def build_jobs():
    ...
    for r in reels:
        ...
        for k in range(3):
            ...
            # prompt: scene k+1 (the start frame's scene), mobile vertical
            prompt_m = r['scenes'][k]
...
def main():
    ...
    for j in jobs:
        ...
        try:
            ...
            log(f"SUBMIT_FAIL {j['prefix']}: {e.read().decode()[:400]}")
    ...
    def do_assemble(rid):
        ...
        try:
            m,mt=assemble(r, CLIPS_M, OUT_M, 1080, 1920)
            d,dt=assemble(r, CLIPS_D, OUT_D, 1920, 1080)
    ...
    log(f"V2 COMPLETE clips_ok={len(active)-len(errs)}/{len(active)} errors={errs} total={int(time.time()-t0)}s")
